...
import {
  prettyOutput,
  queryAllDescendents
} from 'utils/test-utils/platform-utils'
...
describe('test utils: prettyOutput', () => {
  it('outputs simple tree as text', () => {
    ...
    const expectedStrings =
      Platform.OS === 'web' ? expectedWebStrings : expectedNativeStrings
...
describe('test utils: queryAllDescendents', () => {
  it('gets matching descendents', () => {
    ...
    const outerMatches = queryAllDescendents(container, {
      value: 'outer',
      ...idTypes
    })
    expect(outerMatches).toHaveLength(1)

    const childlessMatches = queryAllDescendents(container, {
      value: 'childless',
      ...idTypes
    })
    expect(childlessMatches).toHaveLength(1)

    const innerMatches = queryAllDescendents(outerMatches[0], {
      value: 'inner',
      ...idTypes
    })
    expect(innerMatches).toHaveLength(1)

    const noMatches = queryAllDescendents(childlessMatches[0], {
      value: 'inner',
      ...idTypes
    })
    ...
    const wildcardMatches = queryAllDescendents(container, {
      value: 'er',
      matchType: '$',
      ...idTypes
    })
